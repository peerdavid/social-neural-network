...
def batch_norm(inputs,
  ...
  with tf.variable_scope(scope, 'BatchNorm', [inputs], reuse=reuse):
    ...
    def evaluation(trainable):
      # Just use the moving_mean and moving_variance.
      mean = moving_mean
      variance = moving_variance
      return mean, variance

    mean, variance = tf.cond(is_training, lambda: training(trainable), lambda: evaluation(trainable))
...
def dropout(inputs, keep_prob=0.5, is_training=True, scope=None):
  ...
  # It is possible, that the input is a python bool, or a tensorflow cond. So we have to handle both cases
  return tf.cond(is_training, lambda: training(keep_prob), lambda: evaluation())
